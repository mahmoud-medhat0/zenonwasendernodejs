...
class WAClient {
    ...
    async sendMessage(sessionId, message, phoneNumber,phoneNumber2) {
        ...
        if (status === 'readyforsendmessage') {
            try {
                ...
                if(isRegistered){
                    let messageSent = await client.sendMessage(phoneNumber + '@c.us', message);
                    console.log(`Message sent: ${message}`);   
                    let waSendedMessages = new WaSendedMessages();
                    await waSendedMessages.create({ wa_session_id: sessionId, message: message,message_id:messageSent.id.id,phone_number:phoneNumber });
                    this.resetSessionTimeout(sessionId);
                    return { sessionId, message, phoneNumber, status: 'sent' ,messageSent:messageSent.id.id};
                }
                else if(!isRegistered){
                    let messageSent = await client.sendMessage(phoneNumber2 + '@c.us', message);
                    console.log(`Message sent: ${message}`);   
                    let waSendedMessages = new WaSendedMessages();
    ...
    async CheckNumberIsRegistered(sessionId,phoneNumber) {
        ...
        const isRegistered = await client.getNumberId(phoneNumber);
        return isRegistered;
    }
}
